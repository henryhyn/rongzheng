...
<<results='asis'>>=
sql = "select substr(股票代码,1,6),股票简称,地域,行业编码,年薪,职位,高管姓名 from RZ_Rank_Top limit 0,20"
...
<<results='asis'>>=
sql = "select substr(股票代码,1,6),股票简称,地域,行业编码,年薪,职位,高管姓名 from RZ_Rank_Top limit 20,20"
dt = fetch(dbSendQuery(connect,sql))
data = data.frame(attr(dt,'row.names'),dt)
names(data) = c("排名","股票代码","股票简称","地域","行业编码","年薪(元)","职位","姓名")
data.table = xtable(data,digits=0)
caption(data.table)= paste(this.year," 年非国资委央企\\textbf{最高}年薪排行前 20 位 (有效样本",data.salary$最高年薪[3],"家)")
print(data.table,table.placement="!htbp",caption.placement="top",include.rownames=FALSE)
@

<<results='asis'>>=
sql = "select substr(股票代码,1,6),股票简称,地域,行业编码,年薪,职位,高管姓名 from RZ_Rank_Top limit 40,10"
dt = fetch(dbSendQuery(connect,sql))
data = data.frame(attr(dt,'row.names'),dt)
names(data) = c("排名","股票代码","股票简称","地域","行业编码","年薪(元)","职位","姓名")
data.table = xtable(data,digits=0)
...
for (i in 1:6) {
  ...
  sql = paste("select substr(股票代码,1,6),股票简称,地域,行业编码,高管姓名,年薪 from RZ_Rank_Salary limit ",20*(i-1),",20")
  dt = fetch(dbSendQuery(connect,sql))
  data = data.frame(attr(dt,'row.names'),dt)
...
for (i in 1:5) {
  职位内容 = iterator$职位内容[i]
  职位秩序 = iterator$职位秩序[i]
  sql = paste("select substr(股票代码,1,6),股票简称,地域,行业编码,高管姓名,持股数量,持股市值 from RZ_Rank_Stock limit ",20*(i-1),",20")
  dt = fetch(dbSendQuery(connect,sql))
  data = data.frame(attr(dt,'row.names'),dt)
...
\newpage
\subsection{年度最大升幅升值排行榜前 20 位}
<<results='asis'>>=
sql = "select substr(股票代码,1,6) as 股票代码,股票简称,地域,行业编码,今年年薪,去年年薪,升幅,职位,高管姓名
	from RZ_Salary_Increase order by 升幅 desc limit 0,35"
data = fetch(dbSendQuery(connect,sql))
data.table = xtable(data,digits=c(0,0,0,0,0,0,0,8,0,0))
caption(data.table)= paste(this.year," 年度年薪\\textbf{最大升幅}排行前 20 位")
print(data.table,table.placement="!htbp",caption.placement="top",include.rownames=FALSE)
@

<<results='asis'>>=
sql = "select substr(股票代码,1,6) as 股票代码,股票简称,地域,行业编码,今年年薪,去年年薪,升值,职位,高管姓名
	from RZ_Salary_Increase order by 升值 desc limit 0,35"
data = fetch(dbSendQuery(connect,sql))
data.table = xtable(data,digits=0)
caption(data.table)= paste(this.year," 年度年薪\\textbf{最大升值}排行前 20 位")
print(data.table,table.placement="!htbp",caption.placement="top",include.rownames=FALSE)
@

\newpage
\subsection{年度最大降幅降值排行榜前 20 位}
<<results='asis'>>=
sql = "select substr(股票代码,1,6) as 股票代码,股票简称,地域,行业编码,今年年薪,去年年薪,升幅,职位,高管姓名
	from RZ_Salary_Increase order by 升幅 asc limit 0,35"
data = fetch(dbSendQuery(connect,sql))
data.table = xtable(data,digits=c(0,0,0,0,0,0,0,8,0,0))
caption(data.table)= paste(this.year," 年度年薪\\textbf{最大降幅}排行前 20 位")
print(data.table,table.placement="!htbp",caption.placement="top",include.rownames=FALSE)
@

<<results='asis'>>=
sql = "select substr(股票代码,1,6) as 股票代码,股票简称,地域,行业编码,今年年薪,去年年薪,升值,职位,高管姓名
	from RZ_Salary_Increase order by 升值 asc limit 0,35"
data = fetch(dbSendQuery(connect,sql))
data.table = xtable(data,digits=0)
caption(data.table)= paste(this.year," 年度年薪\\textbf{最大降值}排行前 20 位")
print(data.table,table.placement="!htbp",caption.placement="top",include.rownames=FALSE)
@
